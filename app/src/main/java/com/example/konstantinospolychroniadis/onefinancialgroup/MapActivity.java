...
public class MapActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        googleMap.addMarker(new MarkerOptions().position(paris).title("Properties in Paris"));
        googleMap.moveCamera(CameraUpdateFactory.newLatLng(paris));
        LatLng london = new LatLng(51.509865, -0.118092);
        googleMap.addMarker(new MarkerOptions().position(london).title("Properties in London"));
        googleMap.moveCamera(CameraUpdateFactory.newLatLng(london));
        LatLng brussels = new LatLng(50.85045, 4.34878);
        googleMap.addMarker(new MarkerOptions().position(brussels).title("Properties in Brussels"));
